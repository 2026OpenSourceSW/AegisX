...
describe('DocumentTitle', () => {
    it('renders APP_NAME when no matched route exposes a title handle', async () => {
        ...
        await waitFor(() => expect(document.title).toBe('AegisX'));
    });
    ...
    it('renders a static title from handle', async () => {
        ...
        await waitFor(() => expect(document.title).toBe('Dashboard — AegisX'));
    });
    ...
    it('renders a derived title from a handle.title function reading params', async () => {
        ...
        await waitFor(() => expect(document.title).toBe('Agent Selector — AegisX'));
    });
    ...
    it('renders the deepest matching handle (child wins over parent)', async () => {
        ...
        await waitFor(() => expect(document.title).toBe('API tokens — AegisX'));
    });
    ...
    it('renders a title component produced by apolloTitle()', async () => {
        ...
        await waitFor(() => expect(document.title).toBe('Custom #42 thing — AegisX'));
    });
    ...
    it('treats an unmarked function as a plain resolver, not a component', async () => {
        ...
        // wraps the returned string with the standard "X — AegisX" template.
        ...
        await waitFor(() => expect(document.title).toBe('Item 7 — AegisX'));
    });
